...
def create_aggregated_ranking():
    ...
    for i in range(1,nruns):
        scores = combined[["score_" + str(i)]].to_numpy()
        ranking_values = scipy.stats.rankdata(-scores, method='dense')
        _ranking['ranking_' + str(i)] = ranking_values.tolist()
        # Concatenate the 'genes' column from 'combined' with 'ranking', reset the index, and set 'genes' as the index
        ranking = pd.concat([combined['genes'], _ranking.reset_index(drop=True)], axis=1).set_index('genes')
   
    # Aggregate rankings
    aggr_ranking = rky.borda(ranking, axis=1)
    
    # Convert the Series to a DataFrame
    aggr_ranking_df = aggr_ranking.to_frame()
    
    # Reset the index to turn the current index into a column
    aggr_ranking_df.reset_index(inplace=True)
    
    # Rename the columns appropriately
    aggr_ranking_df.columns = ['genes', 'aggr_ranking']  # Assuming 'ranking' is the name of the series
    
    # Set the 'ranking' column as the index
    aggr_ranking_df.set_index('aggr_ranking', inplace=True)
    
    # Sort the DataFrame by its index
    aggr_ranking_df.sort_index(inplace=True)

    return aggr_ranking_df
...
def main():
    ...
    ensID_top30TFs = aggr_ranking["genes"].head(30).tolist()  # Get top 30 transcription factors based on ranking
